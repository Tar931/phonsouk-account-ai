...
# --- 2. ສ່ວນປ້ອນຂໍ້ມູນ (ຮັກສາຈຸດຕົວເລກໄວ້ຄືເກົ່າ) ---
...
if st.button("🚀 ບັນທຶກຂໍ້ມູນ ແລະ ໃຫ້ AI ວິເຄາະ", use_container_width=True):
    ...
    st.success(f"ບັນທຶກສຳເລັດເມື່ອ: {now.strftime('%H:%M:%S')}")
    st.rerun()

# --- 3. ສ່ວນ AI ແລະ ຕະລາງ (ຮັກສາ AI ໂຕເດີມ 100%) ---
if os.path.exists(FILE_NAME):
    df = pd.read_csv(FILE_NAME)
    
    # ກວດສອບຖັນເພື່ອບໍ່ໃຫ້ Error
    cols_to_show = ['Income', 'Expense', 'Profit']
