...
st.markdown("""
    <style>
    .money-box { 
        background-color: #002B36; color: #00FFAA; padding: 8px 15px; border-radius: 8px; 
        font-size: 22px; font-weight: bold; text-align: right; border: 2px solid #268BD2; margin-bottom: 20px;
    }
...
# --- 2. ສ່ວນປ້ອນຂໍ້ມູນ (ຮັກສາຈຸດຕົວເລກໄວ້) ---
...
if st.button("🚀 ບັນທຶກຂໍ້ມູນ ແລະ ໃຫ້ AI ວິເຄາະ", use_container_width=True):
    ...
    st.success(f"ບັນທຶກສຳເລັດ!")
    st.rerun()

# --- 3. ສ່ວນ AI ແລະ ຕະລາງ (ໃສ່ຈຸດຕົວເລກໃຫ້ໃນຕະລາງ) ---
if os.path.exists(FILE_NAME):
    ...
    if not data.empty:
        ...
        if 'Food' in data.columns and data['Food'].sum() > 0: st.write(f"📌 **ຊ່ອງທາງປະຢັດ:** ປ້າຈ່າຍຄ່າອາຫານ {data['Food'].sum():,.0f} ກີບ.")
        if 'Sewing' in data.columns and data['Sewing'].sum() > 0: st.write(f"🚀 **ແຜນອາຊີບ:** ລາຍໄດ້ຫຍິບຜ້າ {data['Sewing'].sum():,.0f} ກີບ.")
        st.markdown('</div>', unsafe_allow_html=True)

    # ຕະລາງ Excel ທີ່ໃສ່ຈຸດຕົວເລກໃຫ້ເບິ່ງງ່າຍ (Format currency)
    ...
    cols = ['ວັນທີ_ເວລາ', 'Income', 'Expense', 'Profit']
    st.dataframe(display_df[[c for c in cols if c in display_df.columns]].tail(10), use_container_width=True)

    # --- ປຸ່ມ Reset ລົບຂໍ້ມູນທັງໝົດ ---
    st.markdown("---")
    if st.button("🗑️ ລ້າງຂໍ້ມູນທັງໝົດ (Reset Excel)"):
        if os.path.exists(FILE_NAME):
            os.remove(FILE_NAME)
            st.warning("ລົບຂໍ້ມູນທັງໝົດແລ້ວ! ປ້າສາມາດເລີ່ມບັນທຶກໃໝ່ໄດ້ເລີຍ.")
            st.rerun()