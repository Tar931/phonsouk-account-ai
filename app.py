...
# --- 2. ສ່ວນປ້ອນຂໍ້ມູນ (ຮັກສາຈຸດຕົວເລກ Real-time) ---
...
if st.button("🚀 ບັນທຶກຂໍ້ມູນ ແລະ ໃຫ້ AI ວິເຄາະ", use_container_width=True):
    ...
    new_entry = {
        'ວັນທີ_ເວລາ': now.strftime("%d/%m/%Y %H:%M"), # ນີ້ຄືຖັນທີ່ຈະໂຊ ວັນທີ ແລະ ຊົ່ວໂມງ
    ...
    st.success(f"ບັນທຶກແລ້ວເມື່ອ: {now.strftime('%H:%M:%S')}")
    st.rerun()

# --- 3. ສ່ວນ AI ແລະ ຕະລາງ (ແກ້ໄຂໃຫ້ໂຊວັນທີຊົ່ວໂມງ) ---
if os.path.exists(FILE_NAME):
    ...
    # ຕະລາງ Excel ທີ່ປ້າຕ້ອງການ
    st.markdown("### 📅 ຕະລາງປະຫວັດການບັນທຶກ (Excel)")
    # ຖ້າຂໍ້ມູນເກົ່າບໍ່ມີວັນທີ ໃຫ້ໂຊແບບເກົ່າ ຖ້າມີແລ້ວໃຫ້ໂຊຖັນ ວັນທີ_ເວລາ ນຳ
    display_cols = ['Income', 'Expense', 'Profit']
    if 'ວັນທີ_ເວລາ' in df.columns:
        display_cols = ['ວັນທີ_ເວລາ'] + display_cols
